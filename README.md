...
The recommended way to run it on macOS is natively under launchd (see
[Run natively](#run-natively-recommended) below); a self-contained Docker image
is documented next as an alternative.
...
> of that tax and gets real macOS escalation banners.
...
Requires **Node ≥ 22** and an authenticated **`gh` CLI** on the host. Heavy state
(SQLite `state.db`, repo clones, worktrees, CI logs) roots under
`BABYSIT_DATA_DIR` — the launchd plist sets this to the workspace `./.data/`, the
same dir the Docker path uses, so you can switch paths without re-cloning.
...
`launchd/io.tubi.babysit-agent.plist` with `RunAtLoad` + `KeepAlive`, so it comes
up at login and respawns on crash. Dashboard → http://localhost:4317.

For a foreground dev loop instead of the installed daemon:
