...
**All colors and brand-specific styles live in one file:** `color-palette.md`. Read it before generating any diagram and use it as the single source of truth for all color choices — shape fills, strokes, text colors, evidence artifact backgrounds, everything.
...
Run the **render command given to you in your task instructions** (a Node/TypeScript
renderer), passing the path to your `.excalidraw` file. It outputs a PNG next to
the file. Then use the **Read tool** on that PNG to actually view it.
